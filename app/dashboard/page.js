          ...
          <div className="border-t border-zinc-800 rounded-b-2xl overflow-hidden">
            ...
            >
              {messages.length === 0 ? (
                ...
                return (
                  <MarkdownMessage key={m.id ?? i} content={m.content} className={`max-w-[92%] select-text cursor-text rounded-lg px-3 py-2 text-xs leading-relaxed ${isUser ? "ml-auto bg-zinc-700 text-zinc-100" : "mr-auto bg-zinc-800 text-zinc-200"}`} />
                );
              })}
            </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (activeTeammateMessages.length > 0 && tmChatScrollRef.current) {
...
useEffect(() => {
  ...
  <div className="relative" data-settings-menu>
    ...
    {sidebarOpen ? (
      ...
      <button type="button" title="Settings" onClick={() => setSettingsMenuOpen(o => !o)} className="flex h-8 w-8 mx-auto items-center justify-center rounded-md text-zinc-500 transition-colors hover:bg-zinc-800/60 hover:text-zinc-300"><SettingsIcon /></button>
    )}
  </div>
</div>
          </aside>
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div 
        ...
        {onboardingMessages.map((message, index) => {
                    ...
                    return (
                      <MarkdownMessage key={key} content={message.content} className={`max-w-[92%] select-text cursor-text rounded-lg px-3 py-2 text-sm leading-relaxed ${assistant ? "mr-auto bg-zinc-800 text-zinc-200" : "ml-auto bg-zinc-700 text-zinc-100"}`} />
                    );
                  })}
      </div>
